...
- adiciona consulta oficial do `AdvancedProtectionManager` no Android 16+ e detecta apenas a regressão ativo → desativado como mudança de postura;
- exporta relatório JSON redigido para um arquivo escolhido pelo usuário via Storage Access Framework, omitindo package names reais, detalhes e atributos sensíveis;
- adiciona controle para redefinir somente as linhas de base do monitoramento, preservando histórico, cache OSV e relatório atual;
- explicita no relatório e no JSON a quantidade de limitações de análise (`ANALYSIS_GAP`/`ANALYSIS_LIMIT`);
- corrige a concorrência do GitHub Actions para ser por branch/ref, evitando que builds de branches diferentes se bloqueiem mutuamente;
...
- eleva o ambiente para `compileSdk 36`, `targetSdk 36` e AGP 8.10.1 após auditoria das mudanças do Android 16, incluindo tratamento explícito dos insets de edge-to-edge;
