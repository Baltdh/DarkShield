...
public class StaticApkAnalyzerTest {
    ...
    @Test public void unreadableDexSampleIsReportedAsIncomplete() throws Exception {
        File apk = File.createTempFile("darkshield-test", ".apk");
        try {
            ...
            try (java.io.RandomAccessFile file = new java.io.RandomAccessFile(apk, "rw")) {
                while (file.getFilePointer() + 30 <= file.length()) {
                    ...
                    file.skipBytes(2);
                    file.skipBytes(2);
                    int method = Short.toUnsignedInt(Short.reverseBytes(file.readShort()));
                    file.skipBytes(4);
                    file.skipBytes(4);
                    int compressedSize = Integer.reverseBytes(file.readInt());
                    int nameLength = Short.toUnsignedInt(
                            Short.reverseBytes(file.readShort()));
                    int extraLength = Short.toUnsignedInt(
                            Short.reverseBytes(file.readShort()));
                    byte[] name = new byte[nameLength];
                    file.readFully(name);
                    file.skipBytes(extraLength);

                    String entryName = new String(name, "ISO-8859-1");
                    if (method == ZipEntry.STORED && "classes.dex".equals(entryName)) {
                        long dataOffset = file.getFilePointer();
                        file.seek(dataOffset);
                        int original = file.readUnsignedByte();
                        file.seek(dataOffset);
                        file.writeByte(original ^ 0x01);
                        assertEquals(dex.length, compressedSize);
                        break;
                    }

                    file.seek(offset + 30L + nameLength + extraLength + compressedSize);
                }
            }
            ...
            assertTrue(incomplete.detail.contains(
                    "não deve ser interpretada como ausência de risco"));
        } finally {
            assertTrue(apk.delete());
        }
    }
